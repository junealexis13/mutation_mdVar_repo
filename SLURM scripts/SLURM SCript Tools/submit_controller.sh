...
jobArray=("cpu" "cpu" "cpu" "cpu" "cpu" "cpu" "cpu" "cpu" "cpu" "cpu")
i=0
echo "
	...
	if ! [[ $DIRS == "com_Pr1_C478K" ]]; then
		if [[ -d ./$DIRS ]]; then
			if [[ "${jobArray[$i]}" == "cpu" ]]; then
				...
				sed -i "s/yyyy/$i/g" omic_submit_pt1.sh
				sed -i "s/xxxx/$DIRS/g" omic_submit_pt2.sh
				sed -i "s/yyyy/$i/g" omic_submit_pt2.sh 
				sed -i "s/xxxx/$DIRS/g" continue_from_cpy.sh
				sed -i "s/yyyy/$i/g" continue_from_cpy.sh
				sed -i "s/xxxx/$DIRS/g" wrap_traj.sh
				sed -i "s/yyyy/$i/g" wrap_traj.sh
				sed -i "s/xxxx/$DIRS/g" data_extraction.sh
  				((i++))

				cd ..
			elif [[ "${jobArray[$i]}" == "gpu" ]]; then
